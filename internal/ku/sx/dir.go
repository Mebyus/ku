...
// LoadParams specifies how directory should be scanned for source files.
type LoadParams struct {
	// Path to directory which needs to be scanned.
	Dir string
...
func (p *Pool) LoadDir(params *LoadParams) ([]*Text, error) {
	...
	if len(names) == 0 {
		return nil, fmt.Errorf("directory \"%s\" does not contain suitable ku source files", params.Dir)
	}
	slices.Sort(names)

	texts := make([]*Text, 0, len(names))
	for _, name := range names {
		text, err := p.Load(filepath.Join(params.Dir, name))
		if err != nil {
			return nil, err
		}
